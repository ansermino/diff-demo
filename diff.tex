...
%DIF DEL previous.tex   Thu Sep 19 16:16:49 2019
%DIF ADD latest.tex     Thu Sep 19 16:16:09 2019
...
\date{\DIFdelbegin \DIFdel{August 28}\DIFdelend \DIFaddbegin \DIFadd{September 14}\DIFaddend , 2019}
...
point of view, each extrinsics is simply a \DIFdelbegin \DIFdel{SCALE encoded }\DIFdelend \DIFaddbegin \DIFadd{SCALE-encoded }\DIFaddend byte array (see
...
\ref{sect-msg-transactions}. \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend Upon receiving a Transactions message, Polkadot
...
receiving a transactions message, Polkadot RE \DIFdelbegin \DIFdel{separate }\DIFdelend \DIFaddbegin \DIFadd{separates }\DIFaddend the submitted
transactions in the transactions message into individual extrinsics and \DIFdelbegin \DIFdel{pass
}\DIFdelend \DIFaddbegin \DIFadd{passes
}\DIFaddend them to the Runtime by executing Algorithm \ref{algo-validate-transactions} to
...
\begin{itemizeminus}
  ...
  \DIFdelbegin \DIFdel{runtime }\DIFdelend \DIFaddbegin \DIFadd{Runtime }\DIFaddend entry specified in Section \ref{sect-rte-validate-transaction} and
  Requires(R), Priority(R) and Propagate(R) refer to the corresponding fields
  in the tuple \DIFdelbegin \DIFdel{returns }\DIFdelend \DIFaddbegin \DIFadd{returned }\DIFaddend by the entry when it deems that $T$ is valid.

  \item {\tmname{Provided-Tags}}(T) is the list of tags that transaction $T$
  provides. Polkadot RE needs to keep track of tags \DIFdelbegin \DIFdel{which }\DIFdelend \DIFaddbegin \DIFadd{that }\DIFaddend transaction $T$
  provides as well as requires after validating it.

  \item {\tmname{Insert-At(}}$\tmop{TQ}, T, \tmop{Requires} (R),
  \tmop{Priority} (R)$) places $T$ into $\tmop{TQ}$ \DIFdelbegin \DIFdel{approperiatly }\DIFdelend \DIFaddbegin \DIFadd{approperietly }\DIFaddend such that
...
\begin{definition}
  ...
  $B$}}, {\DIFdelbegin \DIFdel{\tmstrong{$H_h (b)$}}\DIFdelend \DIFaddbegin \DIFadd{\tmstrong{$H_h (B)$}}\DIFaddend }, is the hash of the header of block $B$
  encoded by simple codec:''
  \[ H\DIFdelbegin \DIFdel{_b }\DIFdelend \DIFaddbegin \DIFadd{_h }\DIFaddend (\DIFdelbegin \DIFdel{b}\DIFdelend \DIFaddbegin \DIFadd{B}\DIFaddend ) \assign \tmop{Blake} 2 b (\tmop{Enc}_{\tmop{SC}} (\tmop{Head}
     (B))) \]
\end{definition}
...
\subsection{Block Submission\DIFdelbegin \DIFdel{and Validation}\DIFdelend }
...
Blocks can be handed to the Polkadot RE both from the network stack \DIFaddbegin \DIFadd{for
example by means of Block response network message (see Section
\ref{sect-msg-block-response} ) }\DIFaddend and from the consensus engine.

\DIFaddbegin \subsection{\DIFadd{Block Validation}}

\DIFaddend Both the Runtime and the Polkadot RE need to work together to assure block
validity. \DIFdelbegin \DIFdel{This can be accomplished }\DIFdelend \DIFaddbegin \DIFadd{A block is deemed valid if the block author had the authorship right
...
The latter can be verified }\DIFaddend by Polkadot RE invoking \tmverbatim{execute\_block}
entry into the \DIFdelbegin \DIFdel{runtime }\DIFdelend \DIFaddbegin \DIFadd{Runtime }\DIFaddend as a part of the validation process.
...
{\DIFdelbegin %DIFDELCMD < \algorithmic{{\state{{\tmname{Verify-Block-Justification}}$(B, \tmop{Just}
%DIFDELCMD < (B))$}}
%DIFDELCMD < 

%DIFDELCMD < {\state{{\IF{$B$ {\tmstrong{is}} Finalized {\tmstrong{and}} $P (B)$
%DIFDELCMD < {\tmstrong{is not}} Finalized}}}}
%DIFDELCMD < 

%DIFDELCMD < {\state{{\tmname{Mark-as-Final}}$(P (B))${\END}}}
%DIFDELCMD < 

%DIFDELCMD < {\state{Verify $H_{p (B)} \in \tmop{Blockchain}$}}
%DIFDELCMD < 

%DIFDELCMD < {\state{State-Changes = Runtime{\tmname{$(B)$}}}}
%DIFDELCMD < 

%DIFDELCMD < {\state{{\tmname{Update-World-State}}(State-Changes)}}}%%%
\DIFdelend \DIFaddbegin \algorithmic{{\state{{\IF{$\tmop{Just} (B) \neq \emptyset$}}}}
...
{\state{{\tmname{Update-World-State}}($S$)}}}\DIFaddend }

\hrulefill{\medskip}

For the definition of the finality and the finalized block see Section
\ref{sect-finality}. \DIFaddbegin \DIFadd{$\tmop{PBT}$ is the pruned block tree defined in
Definition \ref{defn-block-tree}. }{\DIFadd{\tmname{Verify-Authorship-Right}}} \DIFadd{is part
...
\begin{definition}
  {\DIFdelbegin \DIFdel{\tmstrong{Block authring session key pair $(\tmop{sk}^s_j,
  \tmop{pk}^s_j)$}}\DIFdelend \DIFaddbegin \DIFadd{\tmstrong{Block authoring session key pair $(\tmop{sk}^s_j,
  \tmop{pk}^s_j)$}}\DIFaddend } is an SR25519 key pair which the block producer
...
\begin{notation}
  ...
  $\tmop{sc}_n$. $\tmop{sc}_n$ is set to the \DIFdelbegin \DIFdel{result of calling runtime entry
  \
  }%DIFDELCMD < \tmverbatim{BabeApi\_slot\_duration} %%%
\DIFdel{at the
  ``}\DIFdelend \DIFaddbegin \tmverbatim{duration} \DIFadd{field in
  the returned data from the call of the Runtime entry
  }\tmverbatim{BabeApi\_epoch} \DIFadd{(see \ref{sect-rte-babeapi-epoch}) at the
  }\DIFaddend beginning of each epoch. For a given block $B$, we use the notation
...
\DIFdel{\tmstrong{Winning threshold}}%DIFDELCMD < } %%%
\DIFdelend \DIFaddbegin \label{defn-winning-threshold}{\tmstrong{Winning threshold}} \DIFaddend denoted by
...
\custombinding{7}{\noindent}\begin{tabular}{l}
  \hline
  \tmtextsc{Algorithm  7. }
  \label{algo-block-production-lottery}{\tmname{Block-production-lottery}}\DIFdelbegin \DIFdel{$(\tmop{sk}^{},
  n)$}\DIFdelend \DIFaddbegin \DIFadd{($\tmop{sk}
  :$session secret key of the producer,
  }

  \DIFadd{$n :$epoch index)}\DIFaddend \\
  \hline
\end{tabular}
...
\begin{definition}
  ...
  notation may be dropped \DIFaddbegin \DIFadd{and B's arrival time is referred to by $T_B$ }\DIFaddend when
...
\begin{definition}
  ...
  \[ (\pi, \DIFdelbegin \DIFdel{S_B}\DIFdelend \DIFaddbegin \DIFadd{d}\DIFaddend , \DIFdelbegin %DIFDELCMD < \tmop{pk}%%%
\DIFdelend \DIFaddbegin \DIFadd{j}\DIFaddend , s, \DIFdelbegin \DIFdel{d}\DIFdelend \DIFaddbegin \DIFadd{w}\DIFaddend ) \]
  in which:

  \begin{center}
    \begin{tabular}{rl}
      \DIFdelbegin \DIFdel{s: }%DIFDELCMD < & %%%
\DIFdel{is the slot at which the block is produced.}%DIFDELCMD < \\
%DIFDELCMD <       %%%
\DIFdelend $\pi, d$: & are the results of the block \DIFdelbegin \DIFdel{lottery }\DIFdelend \DIFaddbegin \DIFadd{lottervrf\_output, vrfy }\DIFaddend for
      slot s. \\
      \DIFdelbegin \DIFdel{$\tmop{pk}_j^s$}\DIFdelend \DIFaddbegin \DIFadd{$j$}\DIFaddend : & is the SR25519 session public key associated with the block
      producer. \\
      \DIFdelbegin \DIFdel{$S_B$}\DIFdelend \DIFaddbegin \DIFadd{s}\DIFaddend : & \DIFdelbegin \DIFdel{$\tmop{Sig}_{\tmop{SR} 25519, \tmop{sk}^s_j}
      (\tmop{Enc}_{\tmop{SC}} (s, \tmop{Black} 2 s (\tmop{Head} (B_{}),
      \pi)))$
    }\DIFdelend \DIFaddbegin \DIFadd{is the slot at which the block is produced.}\\
      \DIFadd{w }& \DIFadd{reserved
...
\DIFaddbegin \DIFadd{The block producer includes $H_{\tmop{Babe}} (B)$ as a log in $H_d (B)$ and
sign $\tmop{Head} (B)$ as defined in Definition \ref{defn-block-signature}
}

\begin{definition}
  \label{block-signature}\DIFadd{The }{\DIFadd{\tmstrong{Block Signature}}} \DIFadd{noted by $S_B$ is
...
\DIFdelbegin \subsection{\DIFdel{Block Validation}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \subsection{Epoch Randomness}\DIFaddbegin \label{sect-epoch-randomness}
\DIFaddend 
...
\DIFaddbegin \subsection{\DIFadd{Verifying Authorship Right}}

\begin{definition}
  \DIFadd{Seal $D_s$ }{\todo{define seal}}
\end{definition}

\DIFadd{When a Polkadot node receives a produced block, it needs to verify if the
...
Algorithm \ref{algo-verify-authorship-right} where:
  ...
  \item \DIFadd{T$_B$ is $B$'s arrival time defined in Definition
  \ref{defn-block-time}.
  }

  \item \DIFadd{$H_d (B)$ is the digest sub-component of $\tmop{Head} (B)$ defined in
  Definition \ref{defn-block-header}.
  }

  \item \DIFadd{$\tmop{AuthorityDirectory}^{\mathcal{E}_c}$ is the set of Authority ID
  for block producers of epoch $\mathcal{E}_c$.
  }

  \item {\DIFadd{\tmname{verify-Slot-Winner}}} \DIFadd{is defined in Algorithm
  \ref{algo-verify-slot-winner}.
}\end{itemizeminus}
\custombinding{11}{\noindent}\begin{tabular}{l}
  \hline
  \tmtextsc{Algorithm  11. }
  \label{algo-verify-authorship-right}{\tmname{Verify-Authorship-Right}}\DIFadd{($\tmop{Head}_s
...
\DIFadd{Algorithm \ref{algo-verify-slot-winner} is run as a part of the verification
process, when a node is importing a block, in which:
}\begin{itemizeminus}
  \item {\DIFadd{\tmname{Epoch-Randomness}}} \DIFadd{is defined in Algorithm
  \ref{algo-epoch-randomness}.
  }

  \item \DIFadd{$H_{\tmop{BABE}} (B)$ is the BABE header defined in Definition
  \ref{defn-babe-header}.
  }

  \item {\DIFadd{\tmname{Verify-VRF}}} \DIFadd{is described in Section \ref{sect-vrf}.
  }

  \item \DIFadd{$\tau$ is the winning threshold defined in
  \ref{defn-winning-threshold}.
}\end{itemizeminus}
\custombinding{12}{\noindent}\begin{tabular}{l}
  \hline
  \tmtextsc{Algorithm  12. }
  \label{algo-verify-slot-winner}{\tmname{Verify-Slot-Winner}}\DIFadd{($B$: the block
...
\DIFadd{$(d_B, \pi_B)$: Block Lottery Result for Block $B$,
}

\DIFadd{$s_n$: the slot number,
}

\DIFadd{$n$: Epoch index
}

\DIFadd{AuthorID: The public session key of the block producer
}

\DIFaddend \subsection{Blocks Building Process}
...
\DIFdelbegin %DIFDELCMD < \custombinding{11}{\noindent}%%%
\DIFdelend \DIFaddbegin \custombinding{13}{\noindent}\DIFaddend \begin{tabular}{l}
  \hline
  \DIFdelbegin %DIFDELCMD < \tmtextsc{Algorithm  11. }
%DIFDELCMD <   %%%
\DIFdelend \DIFaddbegin \tmtextsc{Algorithm  13. }
  \DIFaddend \label{algo-build-block}{\tmname{Build-Block}}($C_{\tmop{Best}}$: The chain
  \DIFaddbegin \DIFadd{where }\DIFaddend at its head\DIFaddbegin \DIFadd{, }\DIFaddend the block to be constructed,

  s: Slot number)\\
  \hline
\end{tabular}

{\DIFdelbegin %DIFDELCMD < \algorithmic{{\state{$P_B \leftarrow_{}${\tmname{Head($C_{\tmop{Best}}$)}}}}
%DIFDELCMD < 

%DIFDELCMD < {\state{$H_h (P_B) \leftarrow${\tmname{Call-Runtime-Entry}}$\left(
%DIFDELCMD < \text{\tmverbatim{block\_hash\_from\_id}}, H_i (P_B) \right)$}}
%DIFDELCMD < 

%DIFDELCMD < {\state{$\tmop{Head} (B) \leftarrow$($H_p \leftarrow H_h (P_B), H_i \leftarrow
%DIFDELCMD < H_i (P_B) + 1, H_r \leftarrow \phi, H_e \leftarrow \phi, H_d \leftarrow
%DIFDELCMD < \phi$)}}
%DIFDELCMD < 

%DIFDELCMD < {\state{{\tmname{Call-Runtime-Entry}}$\left(
%DIFDELCMD < \text{\tmverbatim{initialze\_block}}, \tmop{Head} (_{} B) \right)$}}
%DIFDELCMD < 

%DIFDELCMD < {\state{{\tmname{Call-Runtime-Entry}}$\left(
%DIFDELCMD < \text{\tmverbatim{inherent\_extrinsics}},
%DIFDELCMD < \text{{\tmname{Block-Inherents-Data}}} \right)${\END}}}
%DIFDELCMD < 

%DIFDELCMD < {\state{{\FORIN{$E$}{{\tmname{Inherents-Queue}}}}}}
%DIFDELCMD < 

%DIFDELCMD < {\state{{\tmname{$R \leftarrow$Call-Runtime-Entry}}$\left(
%DIFDELCMD < \text{\tmverbatim{apply\_extrinsic}}, E \right)${\END}}}
%DIFDELCMD < 

%DIFDELCMD < {\state{{\WHILE{{\tmstrong{not }}{\tmname{Block-Is-Full($R$}})
%DIFDELCMD < {\tmstrong{and}} {\tmstrong{not}} {\tmname{End-Of-Slot(s)}}}}}}
%DIFDELCMD < 

%DIFDELCMD < {\state{$E
%DIFDELCMD < \leftarrow$\tmverbatim{}{\tmname{{\tmname{}}Next-Ready-Extrinsic()}}}}
%DIFDELCMD < 

%DIFDELCMD < {\state{$R \leftarrow${\tmname{Call-Runtime-Entry}}$\left(
%DIFDELCMD < \text{\tmverbatim{apply\_extrinsics}}, E \right)$}}
%DIFDELCMD < 

%DIFDELCMD < {\state{{\IF{{\tmstrong{not}} {\tmname{Block-Is-FULL($R$}})}}}}
%DIFDELCMD < 

%DIFDELCMD < {\state{{\tmname{Drop(Ready-Extrinsic-Queue,$E$)}}}}
%DIFDELCMD < 

%DIFDELCMD < {\state{$\tmop{Head} (B) \leftarrow${\tmname{Call-Runtime-Entry}}$\left(
%DIFDELCMD < \text{\tmverbatim{finalize\_block}}, E \right)${\END}}}}%%%
\DIFdelend \DIFaddbegin \algorithmic{{\state{$P_B \leftarrow_{}${\tmname{Head($C_{\tmop{Best}}$)}}}}
...
\text{\tmverbatim{finalize\_block}}, E \right)${\END}}}}\DIFaddend }
...
\begin{definition}
  ...
  \]
  Where:
  {\DIFdelbegin %DIFDELCMD < \center{\begin{tabular}{rll}
%DIFDELCMD <     r: & round number & 64 bit integer\\
%DIFDELCMD <     $V_{\tmop{id}}$: & incremental change tracker counter & 64 bit integer\\
%DIFDELCMD <     {\rightaligned{$v_{\tmop{id}}$}}: & Ed25519 public key of $v$ & 4 byte
%DIFDELCMD <     array\\
%DIFDELCMD <     {\rightaligned{}}$\tmop{stage}$: & 0 if it is the pre-vote sub-round & 1
%DIFDELCMD <     byte\\
%DIFDELCMD <     & 1 if it the pre-commit sub-round & 
%DIFDELCMD <   \end{tabular}}%%%
\DIFdelend \DIFaddbegin \center{\begin{tabular}{rll}
...
\begin{definition}
  ...
  in which either
  \[ B' \DIFdelbegin \DIFdel{> }\DIFdelend \DIFaddbegin \geqslant \DIFaddend B \]
...
\DIFdelbegin %DIFDELCMD < \custombinding{12}{\noindent}%%%
\DIFdelend \DIFaddbegin \custombinding{14}{\noindent}\DIFaddend \begin{tabular}{l}
  \hline
  \DIFdelbegin %DIFDELCMD < \tmtextsc{Algorithm  12. }
%DIFDELCMD <   %%%
\DIFdelend \DIFaddbegin \tmtextsc{Algorithm  14. }
  \DIFaddend \label{alg-join-leave-grandpa}{\tmname{Join-Leave-Grandpa-Voters}}
  ($\mathcal{V}$)\\
  \hline
\end{tabular}
...
by following Algorithm \DIFdelbegin \DIFdel{\ref{alg-grandpa-round}}\DIFdelend \DIFaddbegin \DIFadd{\ref{algo-grandpa-round}}\DIFaddend .

\DIFdelbegin %DIFDELCMD < \custombinding{13}{\noindent}%%%
\DIFdelend \DIFaddbegin \custombinding{15}{\noindent}\DIFaddend \begin{tabular}{l}
  \hline
  \DIFdelbegin %DIFDELCMD < \tmtextsc{Algorithm  13. }
%DIFDELCMD <   %DIFDELCMD < \label{alg-grandpa-round}%%%
{\tmname{Play-Grandpa-round}}%%%
\DIFdelend \DIFaddbegin \tmtextsc{Algorithm  15. }
  \label{algo-grandpa-round}{\tmname{Play-Grandpa-round}}\DIFaddend $(r)$\\
  \hline
\end{tabular}

{\DIFdelbegin %DIFDELCMD < \algorithmic{{\state{$t_{r, v} \leftarrow$Time}}
%DIFDELCMD < 

%DIFDELCMD < {\state{$\tmop{primary} \leftarrow${\tmname{Derive-Primary}}}}
%DIFDELCMD < 

%DIFDELCMD < {\state{{\IF{$v = \tmop{primary}$}}}}
%DIFDELCMD < 

%DIFDELCMD < {\state{{\tmname{Broadcast(}}\left.$M_{v_{\nosymbol}}^{r - 1,
%DIFDELCMD < \tmop{Fin}}$({\tmname{Best-Final-Candidate}}($r$-1))\right){\END}}}
%DIFDELCMD < 

%DIFDELCMD < {\state{{\tmname{Receive-Messages}}({\tmstrong{until}} Time $\geqslant t_{r_,
%DIFDELCMD < v} + 2 \times T$ {\tmstrong{or}} $r$ {\tmstrong{is}} completable){\END}}}
%DIFDELCMD < 

%DIFDELCMD < {\state{$L \leftarrow${\tmname{Best-Final-Candidate}}($r$-1)}}
%DIFDELCMD < 

%DIFDELCMD < {\state{{\IF{{\tmname{Received($M_{v_{\tmop{primary}}}^{r, \tmop{pv}} (B)$)}}
%DIFDELCMD < {\tmstrong{and}} $B^{r, \tmop{pv}}_v \geqslant B > L$}}}}
%DIFDELCMD < 

%DIFDELCMD < {\state{$N \leftarrow B${\END}}}
%DIFDELCMD < 

%DIFDELCMD < {\state{{\ELSE{\ }}}}
%DIFDELCMD < 

%DIFDELCMD < {\state{$N \leftarrow B' : H_n (B') = \max \{H_n (B') : B' > L\} \END$}}
%DIFDELCMD < 

%DIFDELCMD < {\state{{\tmname{Broadcast}}($M_v^{r, \tmop{pv}} (N)$)}}
%DIFDELCMD < 

%DIFDELCMD < {\state{{\tmname{Receive-Messages}}({\tmstrong{until}} $B^{r,
%DIFDELCMD < \tmop{pv}_{\nosymbol}}_v \geqslant L$ {\tmstrong{and}} (Time $\geqslant t_{r_,
%DIFDELCMD < v} + 4 \times T${\tmstrong{ or }}$r$ {\tmstrong{is}} completable))}}
%DIFDELCMD < 

%DIFDELCMD < {\state{{\tmname{Broadcast($M_v^{r, \tmop{pc}}$($B_v^{r, \tmop{pv}}$))}}}}
%DIFDELCMD < 

%DIFDELCMD < {\state{{\tmname{Play-Grandpa-round}}($r + 1$)}}}%%%
\DIFdelend \DIFaddbegin \algorithmic{{\state{$t_{r, v} \leftarrow$Time}}
...
{\state{{\tmname{Play-Grandpa-round}}($r + 1$)}}}\DIFaddend }
...
explained in Algorithm \ref{algo-grandpa-best-candidate} \DIFaddbegin \DIFadd{and
}{\DIFadd{\tmname{{\tmname{Attempt-To-Finalize-Round}}($r$)}}} \DIFadd{is described in Algorithm
\ref{algo-attempt-tofinalize}}\DIFaddend .

\DIFdelbegin %DIFDELCMD < \custombinding{14}{\noindent}%%%
\DIFdelend \DIFaddbegin \custombinding{16}{\noindent}\DIFaddend \begin{tabular}{l}
  \hline
  \DIFdelbegin %DIFDELCMD < \tmtextsc{Algorithm  14. }
%DIFDELCMD <   %%%
\DIFdelend \DIFaddbegin \tmtextsc{Algorithm  16. }
  \DIFaddend \label{algo-grandpa-best-candidate}{\tmname{Best-Final-Candidate}}($r$)\\
  \hline
\end{tabular}
...
\DIFdelbegin %DIFDELCMD < \custombinding{15}{\noindent}%%%
\DIFdelend \DIFaddbegin \custombinding{17}{\noindent}\DIFaddend \begin{tabular}{l}
  \hline
  \DIFdelbegin %DIFDELCMD < \tmtextsc{Algorithm  15. } {%%%
\DIFdel{\tmname{Attempt-To-Finalize-Round}}%DIFDELCMD < }%%%
\DIFdelend \DIFaddbegin \tmtextsc{Algorithm  17. }
  \label{algo-attempt-tofinalize}{\tmname{Attempt-To-Finalize-Round}}\DIFaddend ($r$)\\
  \hline
\end{tabular}

{\DIFdelbegin %DIFDELCMD < \algorithmic{{\state{$L \leftarrow${\tmname{Last-Finalized-Block}}}}
%DIFDELCMD < 

%DIFDELCMD < {\state{$E \leftarrow${\tmname{Best-Final-Candidate}}($r$)}}
%DIFDELCMD < 

%DIFDELCMD < {\state{{\IF{$E \geqslant L$ {\tmstrong{and}} $V^{r - 1,
%DIFDELCMD < \tmop{pc}}_{\tmop{obs} (v)}^{\nosymbol}_{\nosymbol} (E) > 2 / 3
%DIFDELCMD < |\mathbb{V}|$}}}}
%DIFDELCMD < 

%DIFDELCMD < {\state{{\tmname{Last-Finalized-Block}}$\leftarrow E$}}
%DIFDELCMD < 

%DIFDELCMD < {\state{{\IF{$M_v^{r, \tmop{Fin}} (E) \nin${\tmname{Received-Messages}}}}}}
%DIFDELCMD < 

%DIFDELCMD < {\state{{\tmname{Broadcast}}($M_v^{r, \tmop{Fin}} (E)$)}}
%DIFDELCMD < 

%DIFDELCMD < {\state{{\RETURN{{\END}{\END}}}}}
%DIFDELCMD < 

%DIFDELCMD < {\state{{\tmstrong{schedule-call}} {\tmname{Attempt-To-Finalize-Round}}($r$)
%DIFDELCMD < {\tmstrong{when}} {\tmname{Receive-Messages}} }}}%%%
\DIFdelend \DIFaddbegin \algorithmic{{\state{$L \leftarrow${\tmname{Last-Finalized-Block}}}}
...
{\tmstrong{when}} {\tmname{Receive-Messages}} }}}\DIFaddend }
...
\DIFaddbegin \DIFadd{for
  ...
  \item \DIFadd{any round $r$ if the node $n$ is }{\tmem{not}} \DIFadd{a GRANDPA voter.
  }

  \item \DIFadd{only for rounds $r$ for which the the node $n$ has invoked Algorithm
  \ref{algo-grandpa-round} if $n$ is a GRANDPA voter.
}\end{itemizedot}
\DIFaddend Note that all Polkadot relay chain nodes are supposed to listen to GRANDPA
...
\tmfloat{h}{small}{figure}{\begin{tmcode}[cpp]
  ...
  (export "Core\DIFdelbegin \DIFdel{_authorities}\DIFdelend \DIFaddbegin \DIFadd{_execute_block}\DIFaddend " (func \DIFdelbegin \DIFdel{$Core_authorities))
  (export "Core_execute_block" (func $Core_execute}\DIFdelend \DIFaddbegin \DIFadd{$Core_execute_block))
  (export "Core_initialize_block" (func $Core_initialize}\DIFaddend _block))
  (export "\DIFdelbegin \DIFdel{Core_initialise_block" (func $Core_initialise_block))
  (export "Metadata_metadata" (func $}\DIFdelend Metadata_metadata\DIFdelbegin \DIFdel{))
  (export }\DIFdelend " \DIFaddbegin \DIFadd{(func $Metadata_metadata))
  (export "BlockBuilder_apply_extrinsic" (func $}\DIFaddend BlockBuilder_apply_extrinsic\DIFdelbegin \DIFdel{" (func $BlockBuilder_apply_extrinsic))
  (export "BlockBuilder_finalise_block" (func $BlockBuilder_finalise_block))
  (export }\DIFdelend \DIFaddbegin \DIFadd{))
  (export "BlockBuilder_finalize_block}\DIFaddend " \DIFaddbegin \DIFadd{(func $BlockBuilder_finalize_block))
  (export "BlockBuilder_inherent_extrinsics" 
          (func $}\DIFaddend BlockBuilder_inherent_extrinsics\DIFdelbegin \DIFdel{" 
          (func $BlockBuilder_inherent_extrinsics))
  (export "BlockBuilder_check_inherents" (func $}\DIFdelend \DIFaddbegin \DIFadd{))
  (export "}\DIFaddend BlockBuilder_check_inherents\DIFdelbegin \DIFdel{))
  (export }\DIFdelend " \DIFaddbegin \DIFadd{(func $BlockBuilder_check_inherents))
  (export "BlockBuilder_random_seed" (func $}\DIFaddend BlockBuilder_random_seed\DIFdelbegin \DIFdel{" (func $BlockBuilder_random_seed))
  (export "TaggedTransactionQueue_validate_transaction" 
          (func $}\DIFdelend \DIFaddbegin \DIFadd{))
  (export "}\DIFaddend TaggedTransactionQueue_validate_transaction\DIFdelbegin \DIFdel{))
  (export }\DIFdelend " 
          \DIFaddbegin \DIFadd{(func $TaggedTransactionQueue_validate_transaction))
  (export "OffchainWorkerApi_offchain_worker" 
          (func $}\DIFaddend OffchainWorkerApi_offchain_worker\DIFaddbegin \DIFadd{))
  (export "ParachainHost_validators}\DIFaddend " (func \DIFdelbegin \DIFdel{$OffchainWorkerApi_offchain_worker))
  (export "ParachainHost_duty_roster" (func $}\DIFdelend \DIFaddbegin \DIFadd{$ParachainHost_validators))
  (export "ParachainHost_duty_roster" (func $}\DIFaddend ParachainHost_duty_roster))
  (export "ParachainHost_active_parachains" 
          (func \DIFdelbegin \DIFdel{$ParachainHost_active_parachains))
  (export "ParachainHost_parachain_head" (func $}\DIFdelend \DIFaddbegin \DIFadd{$ParachainHost_active_parachains))
  (export "ParachainHost_parachain_status" (func $}\DIFaddend ParachainHost_parachain\DIFdelbegin \DIFdel{_head}\DIFdelend \DIFaddbegin \DIFadd{_status}\DIFaddend ))
  (export "ParachainHost_parachain_code" (func \DIFdelbegin \DIFdel{$ParachainHost_parachain_code))
  (export "GrandpaApi_grandpa_pending_change" 
          (func $}\DIFdelend \DIFaddbegin \DIFadd{$ParachainHost_parachain_code))
  (export "ParachainHost_ingress" (func $ParachainHost_ingress))
  (export "}\DIFaddend GrandpaApi_grandpa_pending_change\DIFdelbegin \DIFdel{))
  (export }\DIFdelend " 
          \DIFaddbegin \DIFadd{(func $GrandpaApi_grandpa_pending_change))
  (export "GrandpaApi_grandpa_forced_change" 
          (func $}\DIFaddend GrandpaApi_grandpa_forced_change\DIFdelbegin \DIFdel{" 
          (func $GrandpaApi_grandpa_forced_change))
  (export "GrandpaApi_grandpa_authorities" 
          (func $}\DIFdelend \DIFaddbegin \DIFadd{))
  (export "}\DIFaddend GrandpaApi_grandpa_authorities\DIFaddbegin \DIFadd{" (func $GrandpaApi_grandpa_authorities))
  (export "BabeApi_startup_data" (func $BabeApi_startup_data}\DIFaddend ))
  (export "\DIFdelbegin \DIFdel{ParachainHost_validators}\DIFdelend \DIFaddbegin \DIFadd{BabeApi_epoch}\DIFaddend " (func \DIFdelbegin \DIFdel{$Core_authorities))

  (export "BabeApi_slot_winning_threshold" 
          (func $BabeApi_slot_winning_threshold))
}\DIFdelend \DIFaddbegin \DIFadd{$BabeApi_epoch))
  (export "SessionKeys_generate_session_keys" 
          (func $SessionKeys_generate_session_keys))
}

\DIFaddend \end{tmcode}}{\label{snippet-runtime-enteries}Snippet to export entries into
...
Pair of Blake2Hash of the blob as element of $\mathbb{B}_{32}\DIFdelbegin \DIFdel{$ and its length
as 64 bit integer.

\subsection{\tmverbatim{Babe\_authorities}}%DIFDELCMD < \label{sect-rte-babe-auth}%%%

This entry serves to report the set of authorities at a given block. It
receives \tmverbatim{block\_id} as an argument; and returns an array of
\tmverbatim{authority\_id}'s.

\subsection{\tmverbatim{Grandpa\_authorities}}%DIFDELCMD < \label{sect-rte-grandpa-auth}%%%
...
\subsection{\tmverbatim{TaggedTransactionQueue\_validate\_transaction}}%DIFDELCMD < \label{sect-rte-validate-transaction}%%%
...
{\tmstrong{Arguments}}:
\begin{itemize}
  \item $}\DIFdelend \DIFaddbegin \DIFadd{$ and its length
as 64 bit integer.
...
{\center{\begin{tabular}{rll}
  ...
  integer\\
  $}\tmop{sc}\DIFadd{_n$: & Slot count of $\mathcal{E}_n$ (see Definition
...
{\tmstrong{Arguments}}:
\begin{itemize}
  \item $}\DIFaddend H_n (B)$: the block number whose final state is where the transaction
