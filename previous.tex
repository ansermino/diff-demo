...
\ref{sect-msg-transactions}.

Upon receiving a Transactions message, Polkadot RE decodes the transaction and
...
receiving a transactions message, Polkadot RE separate the submitted
transactions in the transactions message into individual extrinsics and pass
...
\begin{itemizeminus}
  ...
  in the tuple returns by the entry when it deems that $T$ is valid.
  
  \item {\tmname{Provided-Tags}}(T) is the list of tags that transaction $T$
  provides. Polkadot RE needs to keep track of tags which transaction $T$
  provides as well as requires after validating it.
  
  \item {\tmname{Insert-At(}}$\tmop{TQ}, T, \tmop{Requires} (R),
  \tmop{Priority} (R)$) places $T$ into $\tmop{TQ}$ approperiatly such that
...
\custombinding{15}{\noindent}\begin{tabular}{l}
  \hline
  \tmtextsc{Algorithm  15. } {\tmname{Attempt-To-Finalize-Round}}($r$)\\
  \hline
\end{tabular}
...
\subsection{\tmverbatim{Babe\_authorities}}\label{sect-rte-babe-auth}
